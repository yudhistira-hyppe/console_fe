...
const Insight = ({title, number, diff, isIncreased}) => {
  ...
  return(
      <div className={clsx(classes.borderInBetween,'flex flex-row p-1 justify-content-between')}>
        <div className={clsx(classes.insightLbl, 'flex-1')}>
          {title}
        </div>
        <div className={clsx(classes.insightLbl, 'flex-1 text-center')}>
          {number}
        </div>
        <div className={clsx(classes.insightLbl,'flex flex-row justify-content-between flex-1')}>
          ...
          {isIncreased ? <TrendingUp style={{color : '#8DCD03', fontSize: '20px'}}/> : <TrendingDown style={{color: '#E00930', fontSize: '20px'}}/>}
          <span>
            {isIncreased ? 'Up' : 'Down'}
          </span>
        </div>
  ...
  return (
    <CmtCard className='p-0'>
      <CmtCardContent className='p-2 pb-0'>
        ...
        <div className='mt-3'>
          <Insight title={'Followers'} number={524} diff={20} isIncreased={true}/>
          <Insight title={'Following'} number={524} diff={20} isIncreased={true}/>
          <Insight title={'Total Likes'} number={524} diff={20} isIncreased={true}/>
        </div>
      </CmtCardContent>
    </CmtCard>
  );
