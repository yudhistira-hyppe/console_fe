...
const Voucher = ({ title, subTitle, expDate, voucherCode }) => {
  ...
  return (
    <div className={classes.voucherContainer}>
      <div className={classes.voucherTitleLbl}>{title}</div>
      <div className={clsx(classes.voucherSubtitle, 'mt-1')}>{subTitle}</div>
    ...
    <CmtCard style={{ minHeight: '72vh' }}>
      ...
      <CmtCardContent className={classes.cardContentRoot}>
        <PerfectScrollbar className={classes.scrollbarRoot}>
          {myVoucher.map((value) => (
            <Voucher title={value.title} subTitle={value.subTitle} expDate={value.expDate} voucherCode={value.voucherCode} />
          ))}
        </PerfectScrollbar>
      </CmtCardContent>
    </CmtCard>
