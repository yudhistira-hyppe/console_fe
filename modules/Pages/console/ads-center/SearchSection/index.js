  ...
  return (
    <>
      <Box className={classes.inBuildAppCard} p={5} pt={2} maxWidth={270}>
          ...
          <AccordionDetails style={{ padding: 0 }}>
            <FormGroup onChange={(e) => handleChange('status', e.target.value)}>
              <FormControlLabel
                label={'Tinjau'}
                value="Tinjau"
                control={<Checkbox defaultChecked={false} color="secondary" checked={filter.status.includes('Tinjau')} />}
              />
              <FormControlLabel
                label={'Dijadwalkan'}
                value="Dijadwalkan"
              ...
              <FormControlLabel
                label={'Habis'}
                value="Habis"
                control={<Checkbox defaultChecked={false} color="secondary" checked={filter.status.includes('Habis')} />}
              />
              <FormControlLabel
                label={'Ditangguhkan'}
                value="Ditangguhkan"
        ...
        <Accordion elevation={0} defaultExpanded disableGutters>
          <AccordionSummary expandIcon={<ExpandMoreIcon />} style={{ padding: '0px' }}>
            <Typography style={{ fontSize: '13px' }}>Penggunaan Kredit</Typography>
          </AccordionSummary>
          <AccordionDetails style={{ padding: 0 }}>
            <RadioGroup value={filter.labelCredit} onChange={(e) => handleChange('rangeCredit', e.target.value)}>
              <FormControlLabel
                label={'<= 200 Kredit'}
                value="<= 200 Kredit"
                control={<Radio defaultChecked={false} color="secondary" checked={filter.labelCredit === '<= 200 Kredit'} />}
              />
              <FormControlLabel
                label={'201 - 500 Kredit'}
                value="201 - 500 Kredit"
                control={
                  <Radio defaultChecked={false} color="secondary" checked={filter.labelCredit === '201 - 500 Kredit'} />
                }
              />
              <FormControlLabel
                label={'501 - 750 Kredit'}
                value="501 - 750 Kredit"
                control={
                  <Radio defaultChecked={false} color="secondary" checked={filter.labelCredit === '501 - 750 Kredit'} />
                }
              />
              <FormControlLabel
                label={'751 - 1000 Kredit'}
                value="751 - 1000 Kredit"
                control={
                  <Radio defaultChecked={false} color="secondary" checked={filter.labelCredit === '751 - 1000 Kredit'} />
                }
              />
            </RadioGroup>
          </AccordionDetails>
