    ...
    updateStatus(data).then((res) => {
      if (res?.error) {
        ...
        isSingle
          ...
          : toast.success(`Berhasil mengubah ${data1?.length} status emoji`);
      }
      onConfirm();
    });
  };
  ...
  const handleDelete = () => {
    ...
    updateStatus(data).then((res) => {
      if (res?.error) {
        ...
        isSingle ? toast.success('Berhasil menghapus emoji') : toast.success(`Berhasil menghapus ${data1?.length} emoji`);
      }
      onConfirm();
    });
  };
        ...
        <Box sx={style}>
          <Stack direction="column" alignItems="center">
            ...
            <Typography style={{ fontWeight: 'bold', textAlign: 'center', fontSize: 20 }}>
              {status === 'noneactive' && `Nonaktifkan Emoji`}
