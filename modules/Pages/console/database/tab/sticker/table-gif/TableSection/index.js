          ...
          {selected?.length >= 1 ? (
            <TableCell colSpan={6} style={{ width: 80 }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" paddingRight="20px">
                ...
                <Typography style={{ color: '#00000099' }}>{selected?.length} GIF telah dipilih</Typography>
              </Stack>
            </TableCell>
          ) : (
            <>
    ...
    <Stack flex={1}>
        ...
        <Box flex={1} flexDirection={'column'} justifyContent={'center'} display={'flex'}>
          {loading ? (
            ...
            <Typography style={{ fontFamily: 'Normal' }}>
              Menampilkan {listGif?.totalRow} hasil (
              {listGif?.totalRow >= 1 ? filter.page * 10 + 1 : listGif?.pageNumber_ * 10} -{' '}
              {listGif?.pageRow * (filter.page + 1)} dari {listGif?.totalRow})
            </Typography>
          )}
        </Box>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="basic-table">
          ...
          <TableBody>
            ...
            ) : listGif?.data?.length >= 1 ? (
              listGif?.data?.map((item, i) => (
                <TableRow
                  key={i}
      ...
      {listGif?.totalRow >= 1 && (
        <Stack alignItems="center" my={3} mr={3}>
          <Pagination
            count={(Number(listGif?.totalRow) / Number(listGif?.pageRow)).toFixed(0) || 1}
