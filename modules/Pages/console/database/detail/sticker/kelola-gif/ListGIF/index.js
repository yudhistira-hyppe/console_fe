...
const ListGIF = ({ category }) => {
  const [gifs, setGIF] = useState(dummyGIF);
  const [showModal, setShowModal] = useState(false);

  const handleDragEnd = (result) => {
    if (!result.destination) return;

    const newList = Array.from(gifs);
        ...
        <Stack direction="column" width="100%" maxWidth={280}>
          ...
          <Stack direction="column" gap={1} mt={3}>
            <Button
              ...
              style={{ height: 40 }}
              fullWidth
              disabled={JSON.stringify(gifs) === JSON.stringify(dummyGIF)}
        ...
        <Paper style={{ width: '100%' }}>
          <Stack direction="column" width="100%">
            <Stack direction="row" style={{ width: '100%', borderBottom: '1px solid #21212114', padding: 24 }}>
              <Typography style={{ width: '50%', fontSize: 14, paddingLeft: 40 }}>Nama GIF</Typography>
