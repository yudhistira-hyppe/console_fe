...
function Row(props) {
  ...
  const { row } = props;
  //const [check, setChecked] = useState(row.check);
    ...
    <React.Fragment>
      <TableRow className={classes.root}>
        <TableCell align="center">
          <Checkbox checked={row.check} onChange={(e) => changeCheckedState(e.target.checked)} inputProps={{ 'aria-label': 'primary checkbox' }} />
        </TableCell>
        <TableCell className={classes.nameCell} component="th" scope="row" onClick={() => router.push('/console/monetize/pendaftaran_konten/'+row.email)}>
          <CmtObjectSummary
  ...
  return (
    <>
      ...
      <Table aria-label="collapsible table">
        ...
        <TableBody>
          {usersPengumuman && usersPengumuman.map((row,index) => (
            <Row key={index} row={row} />
          ))}
        </TableBody>
      </Table>
    </TableContainer>
    <Button className={classes.btnSelectAll} variant="contained" color="primary" onClick={() => null}>Pilih Semua</Button>
    <TablePagination
    ...
    <Box className={classes.boxAction} display="flex">
      <Button variant="outlined" color="primary" onClick={() => router.push('/console/help_center/pengumuman/create')}>Batal</Button>
      <Button variant="contained" color="primary" className={classes.btnSecondary} onClick={() => router.push('/console/help_center/pengumuman/create')}>Selesai</Button>
    </Box>
    </>
  );
}
