...
import { useGetListInterestQuery } from 'api/console/utilitas/interest';
...
const TableList = () => {
  ...
  const { data: listInterest, isFetching: loadingInterest } = useGetListInterestQuery(getParams());

  useEffect(() => {
    if (filter.page >= 1 && listInterest?.length < 1) {
        ...
        case 'name':
          return value.length >= 1
            ? prevVal.find((item) => item.parent === kind)
              ? [...prevVal.filter((item) => item.parent !== kind), { parent: kind, value: `Interest (${value})` }]
              : [...prevVal, { parent: kind, value: `Interest (${value})` }]
  ...
  return (
    <Stack direction={'row'} spacing={3}>
      <ModalInterest open={openModal} handleClose={() => setOpenModal(!openModal)} />
      ...
      <Stack direction="column" height="100%" gap={3}>
        ...
        <Button
          ...
          sx={{ height: 40 }}
          disabled={!access?.find((item) => item?.nameModule === 'utilitas_interest')?.acces?.createAcces}>
