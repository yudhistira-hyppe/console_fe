...
const guestActiveCard = ({ title, amount, progress, children, handlePayload }) => {
  ...
  const headerSubTitle = (
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
      <div style={{ fontWeight: 'bold', fontSize: 20, marginTop: 10, color: '#2A8F9C' }}>{numberWithCommas(amount)}</div>
      {progress && (
      ...
      <Stack direction="row" alignItems="center" gap={1} position="relative" style={{ height: 40 }}>
        ...
        <Popover
          ...
          <Box width={200} p="15px 20px" color="#ffffff" bgcolor="#282828" borderRadius="4px">
            Tamu yang aktif menggunakan aplikasi dalam kurun waktu tertentu
          </Box>
        </Popover>
      </Stack>
      <Select
...
export default guestActiveCard;
