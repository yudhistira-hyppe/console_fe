...
const RecentOrders = () => {
  const [tableData, setTableData] = useState(recentOrders);
  ...
  const filterTableData = (event) => {
    ...
    switch (event.value) {
      case getTodayDate(): {
        return setTableData(recentOrders.filter((item) => item.orderDate === event.value));
      }
      ...
      case 'this_week': {
        return setTableData(
          recentOrders.filter(
            (item) => item.orderDate !== getTodayDate() && item.orderDate !== getYesterdayDate(),
          ),
        );
      }
      default:
        return setTableData(recentOrders);
    }
  };
  ...
  return (
    <CmtCard>
      <CmtCardHeader
        className="pt-4"
        title="Saldo Hyppe Rp 10.000.000"
        titleProps={{
          variant: 'h4',
          component: 'div',
